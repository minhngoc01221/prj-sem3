...
export function SpotsManagementContent({ spots: initialSpots, isLoading }: SpotsManagementContentProps) {
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                ...
                <th className="text-left px-6 py-4 text-sm font-semibold text-gray-900">Tên điểm du lịch</th>
                <th className="text-left px-6 py-4 text-sm font-semibold text-gray-900">Vị trí</th>
                <th className="text-left px-6 py-4 text-sm font-semibold text-gray-900">Vùng miền</th>
                <th className="text-left px-6 py-4 text-sm font-semibold text-gray-900">Loại hình</th>
                ...
                <tr key={spot.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div>
                      ...
                      <p className="text-sm text-gray-500 line-clamp-1">{spot.description}</p>
                    </div>
                  </td>
                  <td className="px-6 py-4 text-gray-600">{spot.location}</td>
                  <td className="px-6 py-4">
                    <span className={`inline-flex px-2.5 py-1 rounded-full text-xs font-medium ${
                      spot.region === 'north' ? 'bg-blue-100 text-blue-700' :
                      spot.region === 'central' ? 'bg-orange-100 text-orange-700' :
                      'bg-green-100 text-green-700'
                    }`}>
                      {regionLabels[spot.region]}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-gray-600">{typeLabels[spot.type]}</td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-1">
                      <span className="text-yellow-500">★</span>
                      <span className="font-medium">{spot.rating.toFixed(1)}</span>
                      <span className="text-gray-400 text-sm">({spot.reviewCount})</span>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <button
